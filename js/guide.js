class Guide {
  constructor(arg1, arg2) {
    ...
    //const fishTotalLength = -10 * fishLength; // bound inside
  ...
  update(arg1) {    
    ...
    // check if heading change is too big
    let angChange = this.lastHeading.angleBetween(this.dir);
    const angLimit = 100;
    if(degrees(angChange) > angLimit) {
      angChange = angLimit - degrees(angChange);
    } else if (degrees(angChange) < -angLimit ){
      angChange = -angLimit - degrees(angChange);
    }
    this.dir.rotate(radians(angChange));
    this.lastHeading = this.dir.copy();
